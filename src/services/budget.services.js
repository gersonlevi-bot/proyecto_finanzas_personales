...
export async function createBudgetServices(dataBudget, userId) {
    ...
    await getCategoryByIdServices(category_id, userId);

    const idBudget = await db.transaction(async (trx) => {
        const isBudgetExisting = await findBudgetInDateRange(
            time_start,
            time_end,
            userId,
            category_id,
        ...
        try {
            budget = await saveBudget(
                { amount, time_start, time_end, category_id, user_id: userId },
                trx
            );
        } catch (error) {
...
export async function updateBudgetServices(budgetId, userId, dataBudget) {
    ...
    const isBudgetUpdated = await db.transaction(async (trx) => {
        ...
        try {
            affectedRows = await updateBudgetById(
                budgetId,
                userId,
